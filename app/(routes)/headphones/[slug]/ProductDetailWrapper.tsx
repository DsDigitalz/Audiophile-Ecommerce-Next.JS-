// app/headphones/[slug]/ProductDetailWrapper.tsx
'use client'; 
import React, { useState } from "react";
import Link from "next/link";
import Image from "next/image";
import { motion } from "framer-motion"; // For the scroll effect as requested
...
// Suggested Product Card (Moved from your old file)
const SuggestedProductCard: React.FC<SuggestedProduct> = ({ name, imageSrc, href }) => {
    return (
        <motion.div 
        ...
        >
            ...
            <h3 className="text-lg md:text-xl font-bold uppercase mb-8 text-black ">{name}</h3>
            <Link
                href={href}
...
const ProductDetailWrapper: React.FC<ProductDetailWrapperProps> = ({ 
    ...
    // Get the actions needed for the Cart functionality
    ...
    const handleAddToCart = () => {
        ...
        addItem(
            { 
                ...
                name: product.name, 
                price: product.price, 
                // Using a generic image path for the cart icon display
                image: "/assets/cart/image-xx99-mark-ii.jpg" 
            }, 
            quantity
        );
        
        // 2. Open the cart modal (the key functionality requested)
        openModal(); 
        
        // 3. Optional: Reset quantity
        setQuantity(1); 
    };
        ...
        <main className="max-w-[1440px] mx-auto px-6 md:px-10 lg:px-20 py-8 md:py-12 lg:py-20">
            ...
            >
                ...
                <div className="w-full md:w-1/2 flex flex-col items-center md:items-start text-center md:text-left">
                    {product.newProduct && (
                        <p style={{ color: ORANGE_ACCENT }} className="text-sm tracking-[10px] uppercase mb-4">
                            New Product
                        </p>
                    )}
                    ...
                    <div className="flex items-center space-x-4">
                        {/* Quantity Selector - Now controls local state */}
                        ...
                        {/* ADD TO CART Button - Now triggers the cart modal */}
                        <motion.button
                            onClick={handleAddToCart} // 👈 Calls function to add item AND open modal
                            style={{ backgroundColor: ORANGE_ACCENT }}
            ...
            {/* --- FEATURES AND IN THE BOX (You would wrap these in motion.section for the scroll effect) --- */}
            {/* NOTE: Omitting full content here for brevity; assume your original sections follow */}
            {/* ... Features and In The Box section content ... */}

            {/* --- GALLERY (3-Image Layout) --- */}
            {/* ... Gallery section content ... */}

            {/* --- YOU MAY ALSO LIKE --- */}
            <section className="max-w-[1440px] mx-auto py-16 md:py-24" aria-label="You May Also Like">
